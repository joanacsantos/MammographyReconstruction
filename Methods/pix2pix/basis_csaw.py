...
def main():
    ...
    aligned_images = np.concatenate((images_train_val,images_with_mv_train_val),axis=2) #CCtoMLO
    print(aligned_images.shape)
    del mlo_train, cc_train 

    save_images("datasets/mammo/train/", aligned_images , SOURCE + "_" + str(run), labels_save_train_a, None, None,INVERT, True)

    save_images("datasets/mammo/test/", np.concatenate((images_test,images_with_mv_test),axis=2), SOURCE + "_" + str(run), labels_save_test_a, None, None, INVERT, True)
