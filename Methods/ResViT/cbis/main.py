...
def get_run_shuffle(rep_dir, source, run):
    ...
    return np.load("output/reproducibility/CBIS_DDSM/runs_shuffle.npy", allow_pickle=True)[run]
...
def single_run_process_aligned(run, np_images,label):
    ...
    aligned_images = np.concatenate((images_with_mv_train_val,images_train_val),axis=2) #MLO to CC
    print(aligned_images.shape)
    del mlo_train, cc_train

    images_with_mv_val = aligned_images[852:]
    labels_save_val_b = labels_save_train_b[852:]

    aligned_images = aligned_images[:852]
    labels_save_train_b = labels_save_train_b[:852]

    save_images("datasets/mammo/train/", aligned_images , SOURCE + "_" + str(run), labels_save_train_b, None, None,INVERT, True)
    save_images("datasets/mammo/val/", images_with_mv_val , SOURCE + "_" + str(run), labels_save_val_b, None, None,INVERT, True)
    save_images("datasets/mammo/test/", np.concatenate((images_with_mv_test,images_test),axis=2), SOURCE + "_" + str(run), labels_save_test_b, None, None, INVERT, True)
...
def main():
    ...
    single_run_process_aligned(0,images,label)
